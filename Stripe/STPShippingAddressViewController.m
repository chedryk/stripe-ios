...
- (void)next:(__unused id)sender {
    switch (self.configuration.shippingType) {
        case STPShippingTypeShipping: {
            ...
            method.amount = 10;
            method.currency = @"usd";
            STPShippingMethodsViewController *nextViewController = [[STPShippingMethodsViewController alloc] initWithShippingMethods:@[method] currency:self.currency theme:self.theme];
            nextViewController.delegate = self;
            [self.navigationController pushViewController:nextViewController animated:YES];
            break;
        }
        case STPShippingTypeDelivery:
            [self.delegate shippingAddressViewController:self didFinishWithAddress:self.addressViewModel.address method:nil];
            break;
    }
}
...
- (void)shippingMethodsViewController:(__unused STPShippingMethodsViewController *)methodsViewController
                  didFinishWithMethod:(STPShippingMethod *)method {
    [self.delegate shippingAddressViewController:self didFinishWithAddress:self.addressViewModel.address method:method];
}
